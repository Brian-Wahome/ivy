...
from ivy.functional.frontends.numpy.ndarray.ndarray import _to_bytes_helper
...
def fromstring(string, dtype=float, count=-1, *, sep, like=None):
    ...
    string_to_array = ivy.asarray(split_string, dtype=dtype)
    bytes_string = _to_bytes_helper(string_to_array, order="C")

    return ivy.frombuffer(bytes_string, dtype=dtype, count=count, like=like)
